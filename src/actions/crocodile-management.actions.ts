...
export function createCrocodile(
  ...
  group('Create Crocs', () => {
    ...
    const res = http.post(
      _url,
      payload as {},
      _requestConfigWithTag({ name: 'Create' })
    );
...
export function updateCrocodile(
  ...
  group('Update Croc', () => {
    ...
    const res = http.patch(
      _url,
      payload,
      _requestConfigWithTag({ name: 'Update' })
    );
...
export function deleteCrocodile(
  ...
  group('Delete Croc', () => {
    const delRes = http.del(
      _url,
      null,
      _requestConfigWithTag({ name: 'Delete' })
    );
...
export function queryCrocodiles(_url: string, crocs: Crocodile[]): Crocodile[] {
  ...
  group('Query Crocodiles', () => {
    ...
    const responses = http.batch([
      [
        'GET',
        `${_url}/public/crocodiles/1/`,
        null,
        { tags: { name: 'PublicCrocs' } },
      ],
      [
        'GET',
        `${_url}/public/crocodiles/2/`,
        null,
        { tags: { name: 'PublicCrocs' } },
      ],
      [
        'GET',
        `${_url}/public/crocodiles/3/`,
        null,
        { tags: { name: 'PublicCrocs' } },
      ],
      [
        'GET',
        `${_url}/public/crocodiles/4/`,
        null,
        { tags: { name: 'PublicCrocs' } },
      ],
    ]);
...
export function checkAges(_crocs: Crocodile[], _minAge: number) {
  group('Functional Test: Check Ages', () => {
    ...
    check(ages as number[], {
      'Crocs are older than 5 years of age': (ages) =>
        Math.min(...ages) > _minAge,
    });
  });
}
